      ...
      {sections.map((section, i) => (
        ...
        >
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none select-none overflow-hidden w-full h-full flex items-center justify-center opacity-[0.03]">
            <span className="text-[60vh] font-serif font-thin tracking-tighter uppercase" style={{ color: section.accent }}>
          ...
          <div className={`max-w-xl pointer-events-auto ${i % 2 === 0 ? 'text-left' : 'text-right'}`}>
            <div 
              className="text-[10px] font-light uppercase tracking-[0.6em] mb-6 transition-colors duration-500"
              style={{ color: section.accent }}
            >
              {section.label}
            </div>
            
            <h2 className="text-5xl md:text-8xl font-serif font-thin mb-8 leading-[0.85] tracking-tighter whitespace-pre-line uppercase">
              {section.title.split('\n').map((line, idx) => (
                <span key={idx} className="block last:text-white/40 font-extralight">
                  {line}
                </span>
              ))}
            </h2>
            ...
            <div className={`flex flex-col gap-8 ${i % 2 === 0 ? 'items-start' : 'items-end'}`}>
              <p className="text-white/70 text-base md:text-lg leading-relaxed max-w-sm font-extralight">
                {section.desc}
              </p>
